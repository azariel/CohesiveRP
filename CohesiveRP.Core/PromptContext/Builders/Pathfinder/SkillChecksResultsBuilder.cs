...
namespace CohesiveRP.Core.PromptContext.Builders.Pathfinder
{
    public class SkillChecksResultsBuilder : IPromptContextElementBuilder
    {
        ...
        private string GeneratePromptInjectionForCharacterRoll(ChatCharacterRoll roll, CharacterSheetInstance currentCharacterSheetInstance, CharacterSheetInstancesDbModel characterSheetsInstances)
        {
            ...
            str.AppendLine($"  <actions>{Environment.NewLine}    -{string.Join($"{Environment.NewLine}- ", roll.Guides.Select(g => $"<reasoning>{g.Reasoning}</reasoning>{(string.IsNullOrWhiteSpace(g.ReactionFromOtherCharactersWhenSucceedingSkillCheck) ? string.Empty : $"<reactionFromOtherCharactersWhenSucceedingSkillCheck>{g.ReactionFromOtherCharactersWhenSucceedingSkillCheck}</reactionFromOtherCharactersWhenSucceedingSkillCheck>")}{(string.IsNullOrWhiteSpace(g.ReactionFromOtherCharactersWhenFailingSkillCheck) ? string.Empty : $"<reactionFromOtherCharactersWhenFailingSkillCheck>{g.ReactionFromOtherCharactersWhenFailingSkillCheck}</reactionFromOtherCharactersWhenFailingSkillCheck>")}"))}{Environment.NewLine}  </actions>");
