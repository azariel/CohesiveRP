...
namespace CohesiveRP.Core.PromptContext.Builders.Pathfinder
{
    public class SkillChecksResultsBuilder : IPromptContextElementBuilder
    {
        ...
        private string GeneratePromptInjectionForCharacterRoll(ChatCharacterRoll roll, CharacterSheetInstance currentCharacterSheetInstance, CharacterSheetInstancesDbModel characterSheetsInstances)
        {
            ...
            str.AppendLine($"  <actions>{Environment.NewLine}    -{string.Join($"{Environment.NewLine}- ", roll.Guides.Select(g => $"<reasoning>{g.Reasoning}</reasoning><reactionFromOtherCharactersWhenSucceedingSkillCheck>{g.ReactionFromOtherCharactersWhenSucceedingSkillCheck}</reactionFromOtherCharactersWhenSucceedingSkillCheck><reactionFromOtherCharactersWhenFailingSkillCheck>{g.ReactionFromOtherCharactersWhenFailingSkillCheck}</reactionFromOtherCharactersWhenFailingSkillCheck>"))}{Environment.NewLine}  </actions>");
