...
namespace CohesiveRP.Core.PromptContext.Builders.Directive
{
    public class PromptContextSummaryLongTermBuilder : IPromptContextElementBuilder
    {
        ...
        public PromptContextSummaryLongTermBuilder(IStorageService storageService, PromptContextFormatElement promptContextFormatElement, GlobalSettingsDbModel settings, ChatDbModel chatDbModel, PersonaDbModel personaLinkedToChat, CharacterDbModel[] charactersLinkedToChat)
        {
        ...
        public async Task<(string, IShareableContextLink)> BuildAsync()
        {
            ...
            string output = $"<summary_long_term>{Environment.NewLine}Previous facts, events, speech and actions (Long-Term){Environment.NewLine}";
            foreach (ISummaryEntryDbModel summaryElement in summaryDbModel.LongTermSummaries.Where(w => !string.IsNullOrWhiteSpace(w.Content)))
            {
            ...
            output += $"{Environment.NewLine}</summary_long_term>{Environment.NewLine}";
